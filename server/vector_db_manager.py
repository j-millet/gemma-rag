...
class vector_db_manager:
    ...
    def get_file_loader(self,doc_path):
        file_extension = os.path.splitext(doc_path)[-1]
        if file_extension in [".pdf"]:
            return PDFMinerLoader(doc_path)
    ...
    def process_file(self,doc_path,user_id):
        doc_loader = self.get_file_loader(doc_path)
    ...
    def get_context(self,user_id,query,top_k=5,max_cosine=0.6):
        if user_id not in self.user_stores:
            return []
        docs_with_distances = sorted(self.db.similarity_search_with_score(query,k=top_k,filter={'user_id': user_id}),key=lambda x: x[1])
...
if __name__ == "__main__":
    ...
    db_manager.add_user_store("test2",["./test_pdf.pdf"])
    print("Done!")